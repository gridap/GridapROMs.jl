struct UnCommonParamOperator{T<:NonlinearOperator} <: NonlinearOperator
  operators::Vector{T}
end
...
for T in (:GridapType,:ExtensionSolver,:ODESolver)
  @eval begin
    function Algebra.solve(solver::$T,op::UnCommonParamOperator)
