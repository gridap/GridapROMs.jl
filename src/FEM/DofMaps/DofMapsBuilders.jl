...
Returns a permutation `dof_to_lex` such that `dof_to_lex[dof]` is the
lexicographic rank of `dof` (x fastest, then y, then z, then component),
for a `SingleFieldFESpace` `f` defined on a `CartesianDiscreteModel`. Used to
reindex a plain (natively/Gridap-ordered) `FESpace` into lexicographic dof
order via `Gridap.FESpaces.reindex_free_dof_ids`.
"""
function get_dof_perm(f::SingleFieldFESpace)
  trian = get_triangulation(f)
  model = get_background_model(trian)
  _get_dof_perm(model,f)
end

function _get_dof_perm(model::DiscreteModel,f::SingleFieldFESpace)
  @notimplemented "Background model must be cartesian for the selected dof reordering"
end

function _get_dof_perm(model::CartesianDiscreteModel,f::SingleFieldFESpace)
  fe_dof_basis = get_data(get_fe_dof_basis(f))
  orders = get_polynomial_orders(f)
  cell_dofs_ids = get_cell_dof_ids(f)
  cell_to_bg_cell = get_cell_to_bg_cell(f)
  _get_dof_perm(fe_dof_basis,cell_dofs_ids,cell_to_bg_cell,model,orders,num_free_dofs(f))
end

function _get_dof_perm(fe_basis::AbstractVector{<:Dof},args...)
  @notimplemented "This function is only implemented for Lagrangian dof bases"
end

function _get_dof_perm(fe_basis::AbstractVector{<:LagrangianDofBasis},args...)
  ...
  if sum(cmp) == length(fe_basis)
    _get_dof_perm(b1,args...)
  else
    @notimplemented "This function is only implemented for Lagrangian dof bases"
  end
end

function _get_dof_perm(fe_dof_basis::Fill{<:LagrangianDofBasis},args...)
  _get_dof_perm(testitem(fe_dof_basis),args...)
end

function _get_dof_perm(
  ...
  for (icell,cell) in enumerate(pcells)
    ...
    for node in 1:length(onodes_cell)
      ...
      for comp in 1:ncomps
        ...
        spatial_to_dof[odof] = dof
      end
    end
  end

  dof_to_lex = zeros(Int,nfree)
  rank = 0
  for dof in spatial_to_dof
    if dof > 0
      rank += 1
      dof_to_lex[dof] = rank
    end
...
_cubic_polytope(::Val{d}) where d = @abstractmethod
_cubic_polytope(::Val{1}) = SEGMENT
_cubic_polytope(::Val{2}) = QUAD
_cubic_polytope(::Val{3}) = HEX
