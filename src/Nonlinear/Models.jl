...
"""
    struct SimpleMLP{P,S} <: AbstractNNModel
...
"""
struct SimpleMLP{P<:AbstractVector,S} <: AbstractNNModel
  params::P
  sizes::S    # NTuple{L,Int} with L = nlayers + 1
end

function SimpleMLP(sizes::AbstractVector{Int}; T::Type=Float64)
  n_params = sum(sizes[i+1] * (sizes[i] + 1) for i in 1:length(sizes)-1)
  params = randn(T, n_params) .* T(0.01)
  SimpleMLP(params, Tuple(sizes))
end
...
(m::SimpleMLP)(x::AbstractMatrix) = _mlp_forward(m.params, m.sizes, x)

"""
    train!(mlp::SimpleMLP, x::AbstractMatrix, y::AbstractMatrix;
           epochs=1000, lr=1e-3) -> SimpleMLP
...
"""
function train!(mlp::SimpleMLP, x::AbstractMatrix, y::AbstractMatrix;
                epochs::Int=1000, lr::Real=1e-3)
...
that constructs and trains a [`SimpleMLP`](@ref) on the provided training
...
function build_mlp_factory(;
  ...
  function factory(r::Realisation, coeff::ConsecutiveParamArray)
    ...
    mlp   = SimpleMLP(sizes; T=eltype(x))
