...
x₀(μ) = interpolate_everywhere([0.0,0.0,0.0],X(μ,t₀))
v₀(μ) = interpolate_everywhere([0.0,0.0,0.0],X(μ,t₀))
a₀(μ) = interpolate_everywhere([0.0,0.0,0.0],X(μ,t₀))
...
# fesnaps, = solution_snapshots(rbsolver,op,(x₀,v₀,a₀))
# rbop = reduced_operator(rbsolver,op,fesnaps)
# μon = realisation(op;nparams=10,sampling=:uniform)
# x̂,rbstats = solve(rbsolver,rbop,μon,(x₀,v₀,a₀))
# x,festats = solution_snapshots(rbsolver,op,μon,(x₀,v₀,a₀))
