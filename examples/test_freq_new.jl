...
state_reduction = Reduction(tol,energy;nparams,sketch=:sprn,compression=:local,ncentroids=10)
rbsolver = RBSolver(solver,state_reduction;nparams_res,nparams_jac)

dir = datadir("khaba_freq_ord2_new")

# fesnaps = load_snapshots(dir)
# x = load_snapshots(dir;label=online_label)
...
# # dgn = rom_diagnostics(dir,rbsolver,op,μon)
# slvr = RBSteady.set_params(rbsolver;nparams=10)
# s = load_snapshots(dir;label=online_label)
# rbop = load_operator(joinpath(dir,"0.0001"),op)
# proj_err = projection_error(slvr,rbop,s)
# jacs = load_jacobians(dir,slvr,op,s;label=online_label)
# ress = load_residuals(dir,slvr,op,s;label=online_label)
# # err_res,err_jac = hr_error(slvr,rbop,ress,jacs,s)
# μ = get_realisation(s)
# u = get_param_data(s)|> similar
# fill!(u,zero(eltype2(u)))
# err_res = RBSteady.hr_error_res(rbop,ress,μ,u)
# err_jac = RBSteady.hr_error_jac(rbop,jacs,μ,u)

# nlop = parameterise(rbop,μ)
# red_jac = RBSteady.diagnostic_jacobian(nlop,u)