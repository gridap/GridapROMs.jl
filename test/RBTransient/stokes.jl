...
g(μ,t) = x -> VectorValue(-μ[2]*x[2]*(1.0-x[2])*sin(2*pi*t/μ[3]),0.0)*(x[1]==0.0)
gμt(μ,t) = TransientParamFunction(g,μ,t)
...
if method == :pod
  ...
  state_reduction = HighOrderReduction(coupling,fill(tol,4),energy;nparams,sketch,compression,ncentroids)
end
...
# local
red_loc = HighOrderReduction(coupling,fill(tol,4),energy;nparams,sketch,compression,ncentroids=1)
