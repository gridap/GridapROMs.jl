...
if method == :pod
  state_reduction = HighDimReduction(tol,energy;nparams,sketch,compression)
elseif method == :ttsvd
  state_reduction = HighDimReduction(fill(tol,3),energy;nparams,sketch,compression)
end
...
jj = jacobian_snapshots(rbsolver,feop,fesnaps)
rr = residual_snapshots(rbsolver,feop,fesnaps)
# rbop = reduced_operator(rbsolver,feop,fesnaps)
