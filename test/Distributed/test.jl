...
vecdata = collect_cell_vector(V0,b(μ,v))
assem = SparseMatrixAssembler(Uμ,V0)
assemμ = parameterize(assem,μ)

A = allocate_matrix(assemμ,matdata)
@test FESpaces.num_cols(assemμ) == size(A,2)
@test FESpaces.num_rows(assemμ) == size(A,1)
assemble_matrix!(A,assemμ,matdata)
assemble_matrix_add!(A,assemμ,matdata)
A = assemble_matrix(assemμ,matdata)
@test FESpaces.num_cols(assemμ) == size(A,2)
@test FESpaces.num_rows(assemμ) == size(A,1)
B = allocate_vector(assemμ,vecdata)
@test FESpaces.num_rows(assemμ) == length(B)
assemble_vector!(B,assemμ,vecdata)
assemble_vector_add!(B,assemμ,vecdata)
B = assemble_vector(assemμ,vecdata)
@test FESpaces.num_rows(assemμ) == length(B)


U0 = HomogeneousTrialFESpace(Uμ)
